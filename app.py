...
def index():
    ...
    if request.method == "POST":
        ...
        fwdpkts = request.form.get("fwdpkts", "").strip()
        bwdpkts = request.form.get("bwdpkts", "").strip()
        duration = request.form.get("duration", "").strip()
        fwdbytelen = request.form.get("fwdbytelen", "").strip()
        bwdbytelen = request.form.get("bwdbytelen", "").strip()
        ...
        if rawflow:
            # Rawflow wins → ignore form fields
            flow_text = rawflow
        else:
            # Rawflow empty → require ALL fields
            required = [srcip, dstip, protocol, fwdpkts, bwdpkts, duration, fwdbytelen, bwdbytelen]
            if not all(required):
                return render_template(
                    "index.html",
                    error="Rawflow empty → all form fields required.",
                    srcip=srcip, dstip=dstip, protocol=protocol,
                    fwdpkts=fwdpkts, bwdpkts=bwdpkts, duration=duration,
                    fwdbytelen=fwdbytelen, bwdbytelen=bwdbytelen,
                    rawflow=rawflow
                )
            ...
            # If rawflow is empty, ALL fields MUST be filled
            if not all(required):
                return render_template(
                    "index.html",
                    error="Rawflow is empty → ALL form fields must be filled.",
                    srcip=srcip, dstip=dstip, protocol=protocol,
                    fwdpkts=fwdpkts, bwdpkts=bwdpkts, duration=duration,
                    fwdbytelen=fwdbytelen, bwdbytelen=bwdbytelen,
                    rawflow=rawflow
        ...
        return render_template("index.html",
                       ...
                       bwdpkts=bwdpkts,
                       duration=duration,
                       fwdbytelen=fwdbytelen,
                       bwdbytelen=bwdbytelen,
                       rawflow=rawflow)
